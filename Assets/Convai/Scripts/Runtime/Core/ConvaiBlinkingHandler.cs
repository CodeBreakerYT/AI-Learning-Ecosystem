...
namespace Convai.Scripts.Runtime.Core
{
    ...
    public class ConvaiBlinkingHandler : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            // A cached index from PlayerPrefs (found on a prior run, possibly against
            // a different/working mesh) can be stale if this character's face mesh
            // reference is broken/reassigned - re-validate against the current mesh's
            // actual blend shape count before trusting it, instead of blindly using an
            // index that no longer exists and crashing SetBlendShapeWeight every blink.
            int currentBlendShapeCount = faceSkinnedMeshRenderer != null && faceSkinnedMeshRenderer.sharedMesh != null
                ? faceSkinnedMeshRenderer.sharedMesh.blendShapeCount
                : 0;
            if (indexOfLeftEyelid >= currentBlendShapeCount) indexOfLeftEyelid = -1;
            if (indexOfRightEyelid >= currentBlendShapeCount) indexOfRightEyelid = -1;

            if (faceSkinnedMeshRenderer != null && currentBlendShapeCount > 0)
            {
            ...
            {
                // Not every character model ships eye-blink blend shapes (confirmed
                // live: some avatars' face mesh genuinely has zero blend shapes) -
                // SetEyelidsBlendShapeWeight below already no-ops safely without
                // them, so this is an expected degradation, not a real error.
                ConvaiLogger.Info("No blink blend shapes found on this character's face mesh - skipping blink animation.", ConvaiLogger.LogCategory.Character);
            }
        ...
        {
            if (faceSkinnedMeshRenderer == null || faceSkinnedMeshRenderer.sharedMesh == null) return;
            int blendShapeCount = faceSkinnedMeshRenderer.sharedMesh.blendShapeCount;
            if (indexOfLeftEyelid >= 0 && indexOfLeftEyelid < blendShapeCount)
                faceSkinnedMeshRenderer.SetBlendShapeWeight(indexOfLeftEyelid, weight);
            if (indexOfRightEyelid >= 0 && indexOfRightEyelid < blendShapeCount)
