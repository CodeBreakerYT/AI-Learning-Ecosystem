...
namespace Convai.Scripts.Runtime.Core
{
    ...
    public class ConvaiGRPCAPI : MonoBehaviour
    {
        ...
        // _cancellationTokenSource is set to null by several cleanup paths
        // (after a call completes/cancels) and only reassigned by specific
        // other call paths afterward - confirmed live that calling
        // SendTriggerData shortly after an active-NPC change can race that
        // recreation, dereferencing a null token and throwing inside
        // SendTriggerData before the server's audio response is ever
        // received (the request itself still sends, so the NPC's text/text-
        // to-speech reply is silently dropped client-side). Lazily
        // recreating here instead of assuming a prior path already did is a
        // minimal, safe guard against that race.
        private CancellationToken EnsureCancellationTokenSource()
        {
            ...
            ActionConfig actionConfig, FaceModel faceModel, string speakerId)
        {
            ...
            try
            {
                ...
                var sendTextToken = EnsureCancellationTokenSource();
                Task receiveResultsTask = Task.Run(
                    async () => { await ReceiveResultFromServer(call, sendTextToken); },
                    sendTextToken);
...
#else
            // UnityEngine.Microphone isn't implemented on WebGL - voice input via Convai's
            // built-in mic pipeline is unavailable there (would need the browser's Web Audio
            // API via a custom jslib bridge, out of scope here since this app doesn't rely
            // on Convai voice interaction for its core Learn Hub flow).
            AudioClip audioClip = null;
            ConvaiLogger.Warn("Microphone recording isn't supported in WebGL builds - voice input is disabled.", ConvaiLogger.LogCategory.Character);
#endif
            ...
            AudioClip audioClip)
        {
            ...
            var streamToken = EnsureCancellationTokenSource();
            Task receiveResultsTask = Task.Run(async () => { await ReceiveResultFromServer(call, streamToken); }, streamToken);
        ...
        public async Task SendTriggerData(ConvaiService.ConvaiServiceClient client, string characterID, TriggerConfig triggerConfig, ConvaiNPC sendingNPC = null)
        {
            ...
            try
            {
                ...
                // Store the task that receives results from the server.
                var triggerToken = EnsureCancellationTokenSource();
                Task receiveResultsTask = Task.Run(
                    async () => { await ReceiveResultFromServer(call, triggerToken, sendingNPC); },
                    triggerToken);
        ...
        public async Task SendFeedback(bool thumbsUp, string interactionID, string feedbackText)
        {
            ...
            try
            {
                ...
                FeedbackResponse response = await _activeConvaiNPC.GetClient().SubmitFeedbackAsync(request, cancellationToken: EnsureCancellationTokenSource());
