...
namespace Convai.Scripts.Runtime.Core
{
    public class ConvaiPlayerInteractionManager : MonoBehaviour
    {
        ...
        public void Initialize(ConvaiNPC convaiNPC, ConvaiCrosshairHandler convaiCrosshairHandler, ConvaiChatUIHandler convaiChatUIHandler)
        {
            ...
            // Crosshair/chat UI only exist when the "Convai Essentials - XR" prefab is
            // in the scene (gaze-crosshair attention targeting, typed chat input) -
            // this project deliberately never includes that prefab (a pre-existing
            // broken prefab reference upstream), so NPCs here rely on TriggerSpeech
            // narration only. Both used to be required, throwing here and aborting
            // the rest of ConvaiNPC.Awake()/InitializeComponents() before lip sync and
            // the actions handler ever got initialized - degrade gracefully instead,
            // same as every other optional-dependency check in this project.
        ...
        private void HandleToggleChat()
        {
            ...
            if (inputFieldInScene != null && !inputFieldInScene.isFocused && _convaiNPC.isCharacterActive)
            {
